...
func RegisterRoutes(cliCtx context.CLIContext, r *mux.Router, storeName string) {
	...
	r.HandleFunc(fmt.Sprintf("/%s/transfers/{%s}", storeName, restName), resolveNameHandler(cliCtx, storeName)).Methods("GET")
	r.HandleFunc(fmt.Sprintf("/%s/transfer/{%s}/whois", storeName, restName), whoIsHandler(cliCtx, storeName)).Methods("GET")
}
